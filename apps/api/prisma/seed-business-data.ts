/**
 * Gerador de dados de negócio para uma empresa (produtos, custos, estoque,
 * fornecedores, ~90 dias de pedidos, 1 compra recebida e 1 conta a pagar).
 * Reutilizado pelo demo-seed e pelo god-seed. Idempotente: limpa os dados de
 * negócio da empresa e recria.
 */
...
export async function seedBusinessData(
  ...
  // Limpeza idempotente dos dados de negócio da empresa.
  ...
  const account = await prisma.marketplaceAccount.create({
    data: {
      companyId,
      marketplaceId: ml.id,
      externalUserId: `demo-ml-${accountSuffix}`,
      nickname: 'LOJA_DEMO',
      accessTokenEnc: '-',
      refreshTokenEnc: '-',
  ...
  for (let i = 0; i < PRODUCTS.length; i++) {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        externalId: `MLB-${accountSuffix}-${p.sku}`,
        sku: p.sku,
        title: p.title,
        brand: p.brand,
        status: 'active',
    ...
    for (let k = 0; k < ordersToday; k++) {
      ...
      const order = await prisma.order.create({
        data: {
          companyId,
          marketplaceAccountId: account.id,
          externalId: `${accountSuffix}-ORD-${orderSeq++}`,
          status: 'PAID',
          currency: 'BRL',
          orderedAt: new Date(now - d * DAY - Math.floor(rnd(0, DAY))),
  ...
  await prisma.purchaseOrderItem.create({
    data: { companyId, purchaseOrderId: po.id, productId: buy.id, quantity: 50, unitCost: money(buy.price * 0.55), receivedQuantity: 50 },
  });
  await prisma.stockMovement.create({
    data: { companyId, productId: buy.id, type: 'ENTRADA', quantity: 50, balanceAfter: 92, unitCost: money(buy.price * 0.55), referenceType: 'purchase_order', referenceId: po.id },
  });
  await prisma.payable.create({
    data: { companyId, supplierId: supA.id, sourceType: 'purchase_order', sourceId: po.id, description: 'Compra recebida', amount: money(buy.price * 0.55 * 50), dueDate: new Date(now + 18 * DAY) },
  });

  const orders = await prisma.order.count({ where: { companyId } });
  return { products: created.length, orders };
}
