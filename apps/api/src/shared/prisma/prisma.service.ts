...
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
   ...
   *
   * Observação importante: o PrismaClient é um Proxy que carrega os delegates de
   * modelo (`user`, `role`, ...). O nome `client` é interceptado por esse Proxy,
   * por isso a propriedade NÃO pode se chamar `client`. Com um nome livre (`db`),
   * o getter é invocado com `this` = Proxy e devolve os delegates corretamente,
   * tanto dentro quanto fora de uma transação.
   */
